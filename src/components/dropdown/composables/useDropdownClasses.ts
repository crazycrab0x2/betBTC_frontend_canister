...
const defaultDropdownClasses =
  'absolute z-10 bg-transparent divide-y divide-gray-100 rounded shadow'

const defaultGapInPx = 8

const placementDropdownAlignmentClasses: Record<DropdownAlignment, string> = {
  horizontal: 'xl:left-[50%] left-0 xl:-translate-x-1/2',
  vertical: 'top-0',
...
export function useDropdownClasses(props: UseDropdownClassesProps): {
  ...
  const contentClasses = computed(() => {
    ...
    return classNames(defaultDropdownClasses, placementDropdownAlignmentClasses[placement])
  })
