...
const searchTransaction = ref<string>('')
...
const onSelectFilter = (filter: any) => {
  ...
  if (filter == 'Yes') {
    filterParam.value = [{ Yes: null }, { SYes: null }]
  } else if (filter == 'No') {
    filterParam.value = [{ No: null }, { SNo: null }]
  } else if (filter == 'Buy') {
    filterParam.value = [{ Yes: null }, { No: null }]
  } else {
    filterParam.value = [{ SYes: null }, { SNo: null }]
  }
  getTransactions()
}
...
const getTransactions = async () => {
  ...
  let transactionActivities = (await actor.get_transaction_activity(
    [Principal.fromText(profile.value.id)],
    [],
    filterParam.value,
    sortParam.value
  )) as Transaction[]
  transactions.value = transactionActivities
