...
import { callGetDelegation, callLogin, callPrepareLogin } from './siwb-provider'
import {
  AddressType,
...
export type SignDataPreparedEvent = { type: 'SIGN_DATA_PREPARED'; data: string }
...
export type AuthenticatedEvent = { type: 'AUTHENTICATED'; data: DelegationIdentity }
...
>(async ({ input: { providerKey }, emit }) => {
  ...
  const { provider, address, network, addresses } = await getRegisterExtension(providerKey)
  if (!provider) {
...
const signMessage = fromPromise<SingatureSettledEvent['data'], SignMessageParams, SigningEvent>(
  async ({ input: { siwbMessage, provider, address, selectedProviderKey }, emit }) => {
    emit({ type: 'SIGNING', data: siwbMessage })
    let signMessageType
    if (selectedProviderKey === 'BitcoinProvider' || selectedProviderKey === 'XverseProviders') {
...
>(async ({ input: { actor, publicKey, address, signMessageType, signature } }) => {
  ...
  states: {
    ...
    connecting: {
      ...
      invoke: {
        src: 'connect',
        input: ({ context, event }) => ({
          providerKey: (event as ConnectEvent)?.providerKey ?? context.providerKey
        }),
        onDone: {
          target: 'preparing',
          actions: [
