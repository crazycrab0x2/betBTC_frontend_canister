...
export class SiwbStorage extends IdbStorage {
  ...
  static async save(identity: Ed25519KeyIdentity, delegation: DelegationChain, address: string) {
    const storage = new this()
    await storage.set(KEY_STORAGE_KEY, JSON.stringify(identity.toJSON()))
    await storage.set(KEY_STORAGE_DELEGATION, JSON.stringify(delegation.toJSON()))
